...
class EventHandlers:
    ...
    def gradio_demo(
        ...
        Optional[str],  # ply file path
        gr.update,  # ply file visibility update
        gr.update,  # ply info visibility update
    ]:
        """
        ...
        if not os.path.isdir(target_dir) or target_dir == "None":
            return (
                ...
                None,  # ply_file
                gr.update(visible=False),  # ply_file visibility
                gr.update(visible=True),  # ply_info visibility
            )
        ...
        ply_file_visible = os.path.exists(ply_path)
        ply_info_visible = not ply_file_visible
        if not ply_file_visible:
            ply_path = None
        ...
        return (
            ...
            ply_path,  # ply_file path
            gr.update(visible=ply_file_visible),  # ply_file visibility
            gr.update(visible=ply_info_visible),  # ply_info visibility
        )
    ...
    def load_example_scene(self, scene_name: str, examples_dir: str = None) -> Tuple[
        ...
        gr.update,
        gr.update,
        Optional[str],  # ply_file path
        gr.update,  # ply_file visibility
        gr.update,  # ply_info visibility
    ]:
        """
        ...
        # Check for cached PLY file
        ply_path = None
        ply_file_visible = False
        ply_info_visible = True
        if target_dir and target_dir != "None":
            ply_file_path = os.path.join(target_dir, "scene.ply")
            if os.path.exists(ply_file_path):
                ply_path = ply_file_path
                ply_file_visible = True
                ply_info_visible = False
                print(f"Loaded cached PLY file: {ply_path}")
        ...
        return (
            ...
            ply_path,
            gr.update(visible=ply_file_visible),
            gr.update(visible=ply_info_visible),
        )
